...
@section('top-menu')
    <li class="js-header-search header-search">
        <form class="form-horizontal" action="{{ url('/apache2/search') }}" method="post">
            {{ csrf_field() }}
...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12 text-right">
            <form action="{{ url('/apache2/') }}" method="get">
                ...
                <div class="block-content">
                    <table class="table table-borderless table-striped table-vcenter">
                        ...
                        @foreach($topSites as $siteName => $value)
                            <tr>
                                <td class="text-left" style="width: 100px;"><a href="{{ url('apache2/'.$siteName) }}"><strong>{{ $siteName }}</strong></a></td>
            ...
            <div class="block block-opt-refresh-icon4">
                <div class="block-header bg-gray-lighter">
                    <div class="block-options">
                        @if($page>1)
                            <a href="{{ url('/apache2/?page='.($page-1)) }}">&laquo; Previous</a>
                            ...
                        @endif
                        <a href="{{ url('/apache2/?page='.($page+1)) }}">Next &raquo;</a>
                    </div>
                    ...
                    <table class="table table-borderless table-striped table-vcenter">
                        ...
                        @foreach($events as $id => $event)
                            <tr>
                                ...
                                <td class="text-left"><a href="{{ url('apache2/'.$event->_source->siteName) }}"><strong>{{ $event->_source->siteName }}</strong></a></td>
                                <td class="hidden-xs">
                                    <dl>
                                        <dt>Source IP</dt>
                                        <dd>{{ $event->_source->apache2->access->remote_ip }}</dd>

                                        <dt>Response Code</dt>
                                        <dd>{{ $event->_source->apache2->access->response_code }}</dd>

                                        <dt>Bytes Sent</dt>
                                        <dd>{{ $event->_source->apache2->access->body_sent->bytes }}</dd>

                                        <dt>Request Method</dt>
                                        <dd>{{ $event->_source->apache2->access->method }}</dd>

                                        <dt>Request URI</dt>
                                        <dd>{{ $event->_source->apache2->access->url }}</dd>

                                        <dt>Referrer</dt>
                                        <dd>{{ $event->_source->apache2->access->referrer }}</dd>

                                        <dt>OS</dt>
                                        <dd>{{ $event->_source->apache2->access->user_agent->os }}</dd>

                                        <dt>User-Agent</dt>
                                        <dd>
                                            {{ $event->_source->apache2->access->user_agent->name }}
                                            {{ (isset($event->_source->apache2->access->user_agent->major))? $event->_source->apache2->access->user_agent->major : '' }}
                                        </dd>

                                        <dt>Country</dt>
                                        <dd>{{ (new League\ISO3166\ISO3166())->alpha2($event->_source->apache2->access->geoip->country_iso_code)['name'] }}</dd>
                                    </dl>
                                </td>
                            </tr>
                        @endforeach
