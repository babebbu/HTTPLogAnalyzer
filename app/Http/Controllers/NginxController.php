...
class NginxController extends Controller
{
	...
	private function queryTopSites ()
	{
		...
		foreach ($topSitesAggregate as $siteName => $value) {
			if($siteName == 'natchapat.tni.chillinghost.com') continue;
			$propertiesQuery = new ElasticSearchQuery('GET', '/filebeat-*/_search', '
		        {
				    ...
				    "aggs" : {
				        "distinct_remote_ip" : {
				            "cardinality" : { "field" : "nginx.access.remote_ip" }
				        },
				        "uniq_country" : {
				            "terms" : { "field" : "nginx.access.geoip.country_iso_code" }
				        }
				    },
				    "query": {
				        "bool": {
		...
		unset($topSitesAggregate['natchapat.tni.chillinghost.com']);
		//dd($topSitesAggregate);
		...
		$this->countries = (new ElasticSearchQuery('GET', '/filebeat-*/_search', '
			{
			    ...
			    "aggs": {
			      "uniq_country" : {
			        "terms" : {
			          "field" : "nginx.access.geoip.country_iso_code"
			        }
			      }
			    }
			}
    ...
    public function index (Request $request)
    {
        ...
        return view('nginx.dashboard')->with([
        	'page' => $page,
        	'events' => $this->events,
        	'range' => $this->range,
    ...
    {
    	return redirect(url('/nginx/'.$request->input('domain')));
    }
}
